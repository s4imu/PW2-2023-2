const loremForm = document.querySelector(
  'form[name="lorem"]'
) as HTMLFormElement;
...
function generateLoremIpsum(event: Event) {
  event.preventDefault();

  const inputElement = loremForm.querySelector(
    'input[type="number"]'
  ) as HTMLInputElement;
...
function generateLoremIpsumParagraphs(numParagraphs: number): string {
  const loremIpsum =
...
function displayLoremIpsum(content: string): void {
